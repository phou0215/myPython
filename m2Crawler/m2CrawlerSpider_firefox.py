...
class SearchCrawl():
    ...
    #
    # # clear cache memory connect chrome setting browser
    # def delete_cache(self):
    #     try:
    #         """Clear the cookies and cache for the ChromeDriver instance."""
    #         # navigate to the settings page
    #         self.driver.get('chrome://settings/clearBrowserData')
    #         btn_deleted = self.get_clear_browsing_button()
    #         wait.until(btn_deleted)
    #         # click the button to clear the cache
    #         btn_deleted.click()
    #         # wait for the button to be gone before returning
    #         wait.until_not(btn_deleted)
    #     except:
    #         self.setPrint('에러 : {}. {}, line: {}'.format(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2].tb_lineno))
    #         return
    ...
    def activate_samsungHome(self, dict_object, name):
        ...
        try:
            ...
            # self.close_window(mode=1)
            sleep(self.delay_time)
            self.action_wait(mode='css', ele_val='div.custom-community-header-middle')
                    ...
                    while True:
                        if flag_end:
                            ...
                            for idx in range(2, article_size + 1):
                                ...
                                board_num = int(board_num)
                                board_date = row.find_element_by_css_selector('div.custom-tile-date > time').text.strip()
                                # 당일 데이터만 획득
                                flag_end = basic_date != board_date
                                if flag_end:
                                    break
                                else:
                                    if board_num in list_bo_no:
