...
class VOCLearner():
    ...
    # grid graph
    def generate_graph(self, cv, total_accuracy):

        x = np.linspace(1, cv, cv)
        self.figure = plt.figure(figsize=(12, 8))
        list_subplot = []
        max_col = 2
        max_row = 3
        for i in range(len(self.dict_model)):
            ax = self.figure.add_subplot(max_row, max_col, i)
            list_subplot.append(ax)

        for ax in enumerate(list_subplot):
            ax.plot(x, total_train_loss, x, total_val_loss, x)
        ax2.plot(x, total_f1, 'b--')

        ax1.set_xlabel('KFold')
        ax2.set_xlabel('KFoil')
    ...
    def generate_data(self):
        ...
        try:
            ...
            self.setPrint('L_type: {}\nP_type: {}\nT_type: {}\nV_type: {}\nTest_Rate: {}'.format(
                self.l_type, self.p_type, self.t_type, self.v_type, self.test_rate
            ))
            ...
            if self.v_type == 2:
                ...
                self.setPrint(self.list_corpus[:10])
    ...
    # make vocabulary
    def generate_vocab(self, list_text):
        try:
            ...
            tf_idf = custom_vector.fit_transform(list_text)
            self.custom_vocab = custom_vector.vocabulary_
            self.setPrint('Custom Vocab {} 개 생성'.format(len(custom_vector.get_feature_names())))
    ...
    def cross_validation(self, dict_model):
        ...
        for idx, (key, model) in enumerate(dict_model.items()):
            ...
            score = cross_validate(model, self.Test_Data_X, self.Test_Data_Y, cv=kfold, scoring="[accuracy")
    ...
    def run(self):
        ...
        try:
            ...
            # 단어장 vector 생성 함수 실행
            if self.v_type == 2:
                self.generate_vocab(self.list_corpus)
